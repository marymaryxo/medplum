...
/** Identifier system for linking appointments in a recurring series. */
export const RECURRENCE_SERIES_IDENTIFIER_SYSTEM = 'urn:medplum:recurrence-series';

export function getRecurrenceSeriesId(appointment: Appointment): string | undefined {
  return appointment.identifier?.find((i) => i.system === RECURRENCE_SERIES_IDENTIFIER_SYSTEM)?.value;
}
...
export async function createEncounter(
  ...
  options?: { recurrenceSeriesId?: string }
): Promise<Encounter> {
  const appointment = await medplum.createResource({
    ...
    ...(options?.recurrenceSeriesId && {
      identifier: [
        { system: RECURRENCE_SERIES_IDENTIFIER_SYSTEM, value: options.recurrenceSeriesId },
      ],
    }),
    participant: [
      {
...
/** Cancels all appointments in a recurring series (and their linked encounters). */
export async function cancelRecurrenceSeries(
  medplum: MedplumClient,
  seriesId: string
): Promise<{ cancelledCount: number }> {
  const appointments = await medplum.searchResources('Appointment', {
    identifier: `${RECURRENCE_SERIES_IDENTIFIER_SYSTEM}|${seriesId}`,
    'status:not': 'cancelled',
  });

  let cancelledCount = 0;
  for (const appointment of appointments) {
    const encounterBundle = await medplum.searchResources('Encounter', {
      appointment: getReferenceString(appointment as Appointment & { id: string }),
    });
    const encounter = encounterBundle[0];
    if (encounter) {
      await updateEncounterStatus(medplum, encounter, appointment as Appointment & { id: string }, 'cancelled');
      cancelledCount++;
    } else {
      // No encounter linked; just cancel the appointment
