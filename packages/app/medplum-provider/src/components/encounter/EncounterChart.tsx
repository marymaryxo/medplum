...
import { cancelRecurrenceSeries, getRecurrenceSeriesId, updateEncounterStatus } from '../../utils/encounter';
...
export const EncounterChart = (props: EncounterChartProps): JSX.Element => {
  ...
  const handleCancelSeries = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      setEncounter((prev) => (prev ? { ...prev, status: 'cancelled' as const } : prev));
      showNotification({
  ...
  }, [medplum, appointment, setEncounter]);
    ...
    <>
      <Stack justify="space-between" gap={0}>
        <EncounterHeader
          ...
          onSign={handleSign}
          onCancelSeries={appointment ? handleCancelSeries : undefined}
          cancelSeriesLoading={cancelSeriesLoading}
        />
        <Box p="md">
          {activeTab === 'notes' && (
            <Stack gap="md">
              ...
              {clinicalImpression && (
                <Card withBorder shadow="sm" mt="md">
                  <Title order={3}>Fill chart note</Title>
                  <Textarea
