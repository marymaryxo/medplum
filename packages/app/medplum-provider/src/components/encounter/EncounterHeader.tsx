...
import type { Appointment, Encounter, Practitioner, Reference } from '@medplum/fhirtypes';
import { IconChevronDown, IconLock, IconLockOpen, IconTrash } from '@tabler/icons-react';
...
export const EncounterHeader = (props: EncounterHeaderProps): JSX.Element => {
  ...
  const recurrenceSeriesId = appointment ? getRecurrenceSeriesId(appointment) : undefined;
  const showCancelSeries = recurrenceSeriesId && status !== 'cancelled' && onCancelSeries;
  ...
  return (
    <>
          ...
          <Group gap="xs">
            ...
            {status === 'cancelled' || status === 'finished' ? (
              <>
                ...
                <Button variant="light" color={getStatusColor(status)} radius="xl" size="md">
      ...
      <Modal opened={confirmSeriesOpened} onClose={closeConfirmSeries} title="Cancel entire series">
        <Text size="md" c="dimmed">
          This will cancel all appointments in this recurring series. This action cannot be undone.
        </Text>
        <Group justify="flex-end" mt="xl" gap="xs">
          <Button onClick={closeConfirmSeries} variant="subtle">
            Keep appointments
          </Button>
          <Button
            color="red"
